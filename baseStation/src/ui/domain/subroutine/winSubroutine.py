...
class WinSubroutine(Subroutine):
    def __init__(self, master, subroutine_runner: ISubroutineRunner,
                 light: Callable[[ttk.Frame], Light]) -> None:
        super().__init__(master, subroutine_runner)
        self._light = light(self)
        self._create_widgets()

    def _create_widgets(self) -> None:
        self._title = ttk.Label(self, text="Switch the state of the light")
        self._button = ttk.Button(self, text="Switch!", command=self._button_click)
    ...
    def _button_click(self) -> None:
        try:
            self._subroutine_runner.execute_switch_light_subroutine(CommandCallback(self._button_click_done))
